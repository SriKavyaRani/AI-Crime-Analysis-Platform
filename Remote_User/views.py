...
def Predict_Crime_Type(request):
    if request.method == "POST":
        df = pd.read_csv("Datasets.csv", encoding='latin1',on_bad_lines='skip')
        
        
        # STOP HERE FOR NOW - Comment out the rest temporarily
        # return render(request, 'RUser/Predict_Crime_Type.html', {'objs': 'Debug mode - check console'})
        ...
        # Rename for clarity
        df.rename(columns={'PREDICT CRIME TYPE DETECTION': 'Label'}, inplace=True)
        ...
        df['results'] = df['Label'].map(label_mapping)
